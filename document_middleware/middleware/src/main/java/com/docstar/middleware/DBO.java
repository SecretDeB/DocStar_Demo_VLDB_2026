...
@Service
public class DBO extends Thread implements AutoCloseable {
    ...
    // multithreaded across the maximum length of file
    private void task33(int threadNum) {
    ...
    private void task12(int threadNum) {
        ...
        for (int i = start; i < end; i++) {
            ...
            // servers_1_2 = BigInteger.valueOf(0); // Test for server correctness for
            // client
    ...
    private BigInteger[] shamirSecretSharingAsBigInt(StringBuilder secret, int serverCount) {
        ...
        // BigInteger slope =
        // BigInteger.valueOf(rand.nextInt(Constant.getMaxRandomBound() -
    ...
    public void task03(long[][] data, int threadNum) {
    ...
    public void task05(long[][][] data, int threadNum) {
        long[] shares = new long[serverCount];
    ...
    public void task06(int threadNum) throws NoSuchAlgorithmException {
        ...
        for (int i = start + phase1Result; i < end + phase1Result; i++) {
            ...
            if ((doubleDataLong[phase1Result][0] / optCol) == (doubleDataLong[i][0] / optCol)) {
                ...
                if (i == phase1Result) {
                    count = doubleDataLong[i][1] + 1; // ← Add this!
                } else {
                    count = doubleDataLong[i][1];
                }
                ...
                for (int j = (int) start_; j < start_ + count; j++) {
                    ...
                    for (int k = 0; k < hash.length(); k = k + Constant.getHashBlockSize()) {
                        ...
                        if (end_ > hash.length())
                            end_ = hash.length();
                        addr[i][2 + Constant.getHashBlockCount() + pointer] = Helper
                                .mod(addr[i][2 + Constant.getHashBlockCount() + pointer] +
    ...
    public void task02(long[] data, int threadNum) {
    ...
    // multithreaded across number of keywords
    private void task11(int threadNum) {
        BigInteger interpolatedValue;
        int batchSize = (int) Math.ceil(keywordCount / (double) numThreads);
        BigInteger[] shares = new BigInteger[serverCount];
        int start = (threadNum - 1) * batchSize;
    ...
    private void task22(int threadNum) {
    ...
    public void task07(BigInteger[][] data, int threadNum) {
    ...
    public void task09(StringBuilder[][] data, int threadNum) {
    ...
    // multithreaded across number of keywords
    private void task13(int threadNum) {
        BigInteger interpolatedValue;
        int batchSize = (int) Math.ceil(keywordCount / (double) numThreads);
        int start = (threadNum - 1) * batchSize;
        int end = start + batchSize;
     ...
     * @param threadNum
     */
    public void task01(BigInteger[] data, int threadNum) {
    ...
    public String revokeGrantAccess(String clientId, String searchKeyword, Boolean access)
        ...
        if (access) { // access grant
            if (!phase1ResultAccess.equals(BigInteger.valueOf(0L))) { // access not present by doing -noaccesscode
                ...
                flag = true;
                System.out.println("Granting access to client " + clientId + " for keyword '" + searchKeyword + "'.");
            }
        } else { // access revoke
            if (phase1ResultAccess.equals(BigInteger.valueOf(0L))) { // access present
                singleData[phase1Result] = noAccessCode[phase1Result];
                flag = true;
                System.out.println("Revoking access of client " + clientId + " for keyword '" + searchKeyword + "'.");
            }
        }
        ...
        for (Map.Entry<String, Integer> entry : keywordIndices.entrySet()) {
            ...
            if (access) { // access grant
                if (!phase1ResultAccess.equals(BigInteger.valueOf(0L))) { // access not present by doing -noaccesscode
                    ...
                    flag = true;
                    System.out
                            .println("Granting access to client " + clientId + " for keyword '" + searchKeyword + "'.");
                }
            } else { // access revoke
                if (phase1ResultAccess.equals(BigInteger.valueOf(0L))) { // access present
                    ...
                    flag = true;
                    System.out
                            .println("Revoking access of client " + clientId + " for keyword '" + searchKeyword + "'.");
                }
            }
            if (!flag) {
    ...
    public Map<String, Set<String>> getClientAccessMap(List<String> keywords)
        ...
        if (allShares[1] == null || allShares[2] == null || allShares[3] == null) {
            System.err.println("ERROR: Missing shares!");
            System.err.println("  Share 1: " + (allShares[1] != null ? "present" : "MISSING"));
            System.err.println("  Share 2: " + (allShares[2] != null ? "present" : "MISSING"));
            System.err.println("  Share 3: " + (allShares[3] != null ? "present" : "MISSING"));
            return new HashMap<>();
        }

        System.out.println("✓ Received all 3 shares correctly");
        ...
        for (int clientId = 1; clientId <= clientCount; clientId++) {
            ...
            System.out.println("Access codes with value 0: " + zeroCount + " / " + clientAccessCodes.length);
            ...
            System.out.println("Client " + clientId + ": " + allowedCount + " allowed keywords");
        }
    ...
    private int getKeywordCountFromServers() throws IOException, ClassNotFoundException {
        System.out.println("Fetching keyword count from servers...");
        ...
        for (int i = 0; i < serverCount; i++) {
            System.out.println("Requesting keyword count from Server " + (i + 1));
        ...
        if (counts[0] == counts[1] && counts[1] == counts[2]) {
            System.out.println("✓ All servers agree on keyword count: " + counts[0]);
            return counts[0];
        } else {
            System.err.println("ERROR: Servers disagree on keyword count!");
            System.err.println("Server 1: " + counts[0]);
            System.err.println("Server 2: " + counts[1]);
            System.err.println("Server 3: " + counts[2]);
    ...
    public Map<String, Integer> getStats() {
        ...
        for (String stat : statNames) {
            ...
            if (stats.get(0).get(stat) == stats.get(1).get(stat) && stats.get(1).get(stat) == stats.get(2).get(stat)) {
                System.out.println("All servers agree on " + stat + ": " + stats.get(0).get(stat));
            } else {
                System.out.println("Servers do not agree on " + stat + ".");
            }
        }
    ...
    public String addNewKeyword(String keyword)
        ...
        System.out.println("Checking if keyword exists...");
        ...
        if (keywordCount > 0) {
            ...
            System.out.println("✓ Keyword does not exist. Proceeding with add operation.");
        }
        ...
        System.out.println("✓ Keyword shares created (4 shares)");
        ...
        System.out.println("✓ Position shares created (position = " + newPosition + ")");

        // Step 3c: Create access shares for all clients
        System.out.println("Creating access shares for " + clientCount + " clients...");
        ...
        for (int i = 0; i < serverCount; i++) {
            ...
            System.out.println("✓ ACT data sent to server " + (i + 1) + " (using share index " + shareIndex + ")");
        }

        System.out.println("\n✓ Step 3 complete (ACT shares created and sent)");

        // ===== STEP 4: RECEIVE CURRENT OPT SIZE FROM SERVERS =====
        System.out.println("\n=== RECEIVING OPT SIZE AND LAST ADDR ===");
        ...
        optRow = receivedOptRow;
        System.out.println("✓ All servers agree: optRow = " + optRow);

        // ===== STEP 5: CREATE AND SEND ADDR DATA =====
        System.out.println("\n=== CREATING ADDR DATA ===");

        // Step 5a: Allocate position in inverted index
        System.out.println("New keyword will start at position: " + newStartPos);
        ...
        for (int i = 0; i < serverCount; i++) {
            ...
            System.out.println("✓ ADDR data sent to server " + (i + 1));
        }
        ...
        keywordCount++;
        System.out.println("Updated keywordCount: " + keywordCount);
        ...
        System.out.println("Updated noAccessCode array length: " + noAccessCode.length);

        // Increment optRow (we allocated a new row)
        /// optRow++;
        System.out.println("Updated optRow: " + optRow);
    ...
    public String bulkUploadFiles(List<FileUpload> files, Set<String> actKeywords)
        ...
        System.out.println("\n" + "=".repeat(80));
        System.out.println("BULK UPLOAD: " + files.size() + " files");
        System.out.println("=".repeat(80));
        ...
        int fileIdToSave = currentFileCount;

        for (FileUpload file : files) {
            if (file.keywords == null || file.keywords.length == 0) {
                continue; // Skip files with no keywords
            }
            docsList.add(new Document((long) (fileIdToSave + 1), file.getFilename(), LocalDateTime.now()));
            fileIdToSave++;
        ...
        System.out.println("✓ Total keyword occurrences: " + totalKeywordOccurrences);
        System.out.println("✓ Unique keywords: " + allUniqueKeywords.size());
        System.out.println("✓ Avg keywords per file: " + (totalKeywordOccurrences / files.size()));

        // Step 1.2: Check which keywords already exist
        System.out.println("\nChecking existing keywords...");
        ...
        System.out.println("✓ Existing keywords: " + (allUniqueKeywords.size() - newKeywords.size()));
        System.out.println("✓ New keywords: " + newKeywords.size());

        // Step 1.3: Batch add ALL new keywords in ONE operation
        if (!newKeywords.isEmpty()) {
            System.out.println("\nBatch adding " + newKeywords.size() + " new keywords...");
            ...
            System.out.println("✓ Batch add completed in " + (batchDuration / 1000.0) + "s");
            System.out.println("✓ Average: " + (batchDuration / newKeywords.size()) + "ms per keyword");
        }

        // Step 1.4: Build keyword index cache for NEW keywords
        System.out.println("\nBuilding keyword index cache...");
        ...
        System.out.println("✓ Cached " + keywordIndexMap.size() + " keyword indices");
    ...
    private void updateAddrDataBatch(int amountAdded, boolean freeSlot)
        ...
        if (freeSlot) {
            ...
            doubleDSharesLong = new long[serverCount][keywordCount][2 + 2 * Constant.getHashBlockCount()];
            stageLabel = "3";
            createThreads(stage, stageLabel); // Generates shares from doubleDataLong
            ...
            createThreads(stage, stageLabel); // Reconstructs doubleDataLong from serverSharesAs
    ...
    public String bulkInitializeSystem(List<FileUpload> files)
        ...
        for (FileUpload file : files) {
            ...
            Document doc = new Document((long) (fileIdToSave + 1), file.getFilename(), LocalDateTime.now());
        ...
        documentRepository.saveAll(
                fileIdToNameMap.entrySet().stream()
                        .map(e -> new Document((long) (e.getKey() + 1), e.getValue(), LocalDateTime.now()))
        ...
        System.out.println("✓ ACT table built: " + numRows + " x " + numKeywords);

        // ========================================================================
        // PHASE 3 & 4: BUILD ADDR AND OPT_INV (Fixed 50k capacity per keyword)
        // ========================================================================
        System.out.println("\n[PHASE 3+4] Building ADDR and inverted index with 50k file capacity...");

        // ✅ Each keyword gets: 8 hash blocks + 50,000 file ID slots
        int SLOTS_PER_KEYWORD = Constant.getHashBlockCount() + 50000; // 8 + 50000 = 50008
        int colSize = SLOTS_PER_KEYWORD; // Each keyword gets one row of 50008 columns

        System.out.println("Configuration:");
        System.out.println("  - Hash blocks per keyword: " + Constant.getHashBlockCount());
        System.out.println("  - File slots per keyword: 50,000");
        System.out.println("  - Total slots per keyword: " + SLOTS_PER_KEYWORD);
        ...
        for (String keyword : uniqueKeywords) {
            ...
            int paddingNeeded = 50000 - filesStored;
            ...
            // Progress logging
            if ((idx + 1) % 1000 == 0) {
                System.out.println("  Processed " + (idx + 1) + "/" + numKeywords + " keywords");
            }
        ...
        for (int col = 0; col < optInvSize; col++) {
            ...
            if ((col + 1) % 1000000 == 0) {
                System.out.println("  Created shares for " + (col + 1) + "/" + optInvSize + " slots");
            }
        }
        ...
        // Send metadata and tables to all servers FIRST
        serverCount = 4;
        for (int s = 0; s < 4; s++) {
            System.out.println("\nInitializing server " + (s + 1) + "...");

            sendToServer("BULK_INITIALIZE", (s + 1));
            ...
            // Send metadata: [fileCount, colSize]
            int[] metadata = { files.size(), colSize };
            sendToServer(metadata, (s + 1));

            // Send ACT
            sendToServer(actShares[shareIndex], (s + 1));

            if (s < 3) {
                // Send ADDR and opt_inv to servers 1-3 only
                sendToServer(addrShares[shareIndex], (s + 1));
                sendToServer(optInvShares[s], (s + 1));
                System.out.println("  ✓ Sent metadata, ACT, ADDR, and optInv to server " + (s + 1));
            } else {
                System.out.println("  ✓ Sent metadata and ACT to server " + (s + 1));
            }
        }
        ...
        for (int batch = 0; batch < totalBatches; batch++) {
            ...
            for (int i = startIdx; i < endIdx; i++) {
                ...
                System.out.println("File " + (i + 1) + " keywords: " + actualKeywordCount + ", padding: " + pad);
                System.out.println("Keyword list length: " + kwList.length);
        ...
        System.out.println("Files: " + files.size());
        System.out.println("Inverted index: " + optRow + " × " + optCol + " = " + optInvSize + " slots");
